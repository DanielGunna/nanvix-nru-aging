...
/*Contador para atualizar a tabela de páginas de tempos em tempos*/
PUBLIC unsigned counter = 0;
...
PRIVATE void do_clock()
{
	...
	counter++;
	//Se o contaodr chegar a 100 quer dizer que houveram 100 ticks ou 2 quantuns
	if(counter == 100){
		//zerar contador para nova contagem
		counter = 0;
		//Atualizar tabela de páginas chamando a função definida em mm.h e implementada em paging.c
		updatePageTable();	
	}
