...
PRIVATE struct
{
	...
	unsigned age;       /**< Age.                 */
	unsigned frequence; /**< Frquencia com que a página é usada.*/
...
void updatePageTable( ){
	//Ponteiro para referenciar tabela de paginas
	struct pte * pag;
	//Loop para percorrer frames
	for (int i  = 0 ; i < NR_FRAMES ; i++){
		
		//Pega o endereço da linha da página do processo corrente na Page Table
		pag = getpte(curr_proc,frames[i].addr);
		//Fazer o processo de aging desloca o contador de frequencia para direita
		frames[i].frequence  = frames[i].frequence >> 1;
		//Pega o bit R da pagina 
 		int aux = pag->accessed;
		//Empurra para o bit mais significativo para fazer incluir no contador
		aux =  aux << 31;
		//Faz um or  para adicionar o bit R ao contador
		frames[i].frequence |= aux;
...
PRIVATE int allocf(void)
{
	...
	//Variavel para guarda o valor i do frame de menor frequencia
	freq = 0xFFFFFFFF;
	...
	{	
		...
		if (frames[i].owner == curr_proc->pid)
		{
			...
			/* Proucurar pela pagina menos frequente. */
			if (freq == 0xFFFFFFFF  || frames[i].frequence < freq)
				freq = i;
		}
	}
